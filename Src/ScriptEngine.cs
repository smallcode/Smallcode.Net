...
namespace Smallcode.Net
{
    public interface IScriptEngine : IDisposable
    {
        ...
        void SetNamedItem(string name, object value);
        ...
        ParsedScript Parse(string text);
    }

    /// <summary>
    ///     Represents a Windows Script Engine such as JScript, VBScript, etc.
    /// </summary>
    public sealed class ScriptEngine : IDisposable, IScriptEngine
    {
        ...
        private interface IActiveScriptParse32
        {
            ...
            void AddScriptlet(string defaultName, string code, string itemName, string subItemName, string eventName,
                              string delimiter, IntPtr sourceContextCookie, uint startingLineNumber, ScriptText flags,
                              out string name, out EXCEPINFO exceptionInfo);

            void ParseScriptText(string code, string itemName, object context, string delimiter,
                                 IntPtr sourceContextCookie, uint startingLineNumber, ScriptText flags,
        ...
        public ScriptEngine(string language)
        {
            ...
            var engine = Type.GetTypeFromProgID(language, true);
        ...
        private object Parse(string text, bool expression)
        {
            ...
            try
            {
                ...
                if (_parse32 != null)
                {
                    ...
                    _parse32.ParseScriptText(text, null, null, null, IntPtr.Zero, 0, flags, out result,
                                             out exceptionInfo);
                }
                else
                {
                    _parse64.ParseScriptText(text, null, null, null, IntPtr.Zero, 0, flags, out result,
                                             out exceptionInfo);
                }
            }
            catch
            ...
            if (expression)
            {
                ...
                if (_parse32 != null)
                {
                    ...
                    var dp = Marshal.GetObjectForIUnknown(dispatch);
                    try
                    {
    ...
    public sealed class ParsedScript : IDisposable
    {
        ...
        public object CallMethod(string methodName, params object[] arguments)
        {
            ...
            try
            {
                return _dispatch.GetType()
                                .InvokeMember(methodName, BindingFlags.InvokeMethod, null, _dispatch, arguments);
            }
            catch
            {
